...
var import_zod, memoryEntrySchema, storeMemoryInputSchema, rememberKindSchema, rememberActionSchema, rememberInputSchema, rememberBatchInputSchema, rememberResultSchema, rememberBatchOptionsSchema, rememberBatchItemResultSchema, rememberBatchResultSchema, metadataFilterValueSchema, metadataFilterOperatorSchema, metadataFilterSchema, queryOptionsSchema, queryResultSchema, queryResponseSchema, defaultMemoryLinkTypes, memoryLinkSchema, memoryLinkInputSchema, linkedMemoryQueryOptionsSchema, queryWithNeighborsOptionsSchema, neighborPathSchema, smartRecallProfileSchema, smartRecallOptionsSchema, smartRecallProfileDefaultsSchema, contextPackSectionTitlesSchema, smartRecallProfileDescriptorSchema, smartRecallResultSchema, smartRecallResponseSchema, dreamMemoryLayerSchema, dreamOptionsSchema, dreamResponseSchema, contextPackOptionsSchema, contextPackSectionSchema, contextPackResponseSchema, memoryHealthOptionsSchema, memoryHealthCheckSchema, memoryHealthRecommendationSchema, memoryHealthResponseSchema, namespaceInputSchema, namespaceQueryScopeSchema, knowledgeResourceUriSchema, knowledgeResourceScopeSchema, knowledgeResourceGrantSchema, knowledgeNodeSchema, knowledgeEdgeSchema, knowledgeGraphArtifactSchema, knowledgeIngestOptionsSchema, knowledgeIngestResultSchema, knowledgeArtifactRegistrationSchema, modelConfigSchema, embeddingConfigSchema, postgresStorageConfigSchema, storageScopeConfigSchema, rememConfigSchema, eventTypeSchema, memoryEventSchema, identityCategorySchema, constitutionStatementSchema, constitutionSchema, driftResultSchema, identityConfigSchema, memoryLayerSchema, layerConfigSchema, layeredMemoryEntrySchema, proceduralTriggerSchema, proceduralMatchSchema, driftEventSchema, identityPackageSchema, duplicationConfigSchema, infectionConfigSchema;
var init_types = __esm({
  "src/types.ts"() {
    "use strict";
    import_zod = require("zod");
    memoryEntrySchema = import_zod.z.object({
    ...
    storeMemoryInputSchema = import_zod.z.object({
    ...
    rememberKindSchema = import_zod.z.enum([
      "fact",
      "preference",
      "decision",
      "procedure",
    ...
    rememberActionSchema = import_zod.z.enum([
      "stored",
      "skipped_duplicate",
      "skipped_low_signal",
      "preview"
    ]);
    rememberInputSchema = storeMemoryInputSchema.extend({
    ...
    rememberBatchInputSchema = import_zod.z.array(rememberInputSchema).min(1);
    rememberResultSchema = import_zod.z.object({
    ...
    metadataFilterValueSchema = import_zod.z.union([import_zod.z.string(), import_zod.z.number(), import_zod.z.boolean(), import_zod.z.null()]);
    metadataFilterOperatorSchema = import_zod.z.object({
    ...
    metadataFilterSchema = import_zod.z.union([metadataFilterValueSchema, metadataFilterOperatorSchema]);
    queryOptionsSchema = import_zod.z.object({
    ...
    queryResultSchema = import_zod.z.object({
    ...
    queryResponseSchema = import_zod.z.object({
    ...
    memoryLinkSchema = import_zod.z.object({
      id: import_zod.z.string().uuid(),
    ...
    memoryLinkInputSchema = import_zod.z.object({
      fromId: import_zod.z.string().uuid(),
      toId: import_zod.z.string().uuid(),
      type: import_zod.z.string().min(1),
      metadata: import_zod.z.record(import_zod.z.unknown()).optional().default({})
    });
    linkedMemoryQueryOptionsSchema = import_zod.z.object({
    ...
    queryWithNeighborsOptionsSchema = queryOptionsSchema.extend({
    ...
    neighborPathSchema = import_zod.z.object({
    ...
    smartRecallProfileSchema = import_zod.z.enum([
      "fast",
      "deep",
      "agent-safe",
      "ops-debug",
    ...
    smartRecallOptionsSchema = queryWithNeighborsOptionsSchema.extend({
    ...
    smartRecallProfileDefaultsSchema = import_zod.z.object({
    ...
    contextPackSectionTitlesSchema = import_zod.z.object({
    ...
    smartRecallProfileDescriptorSchema = import_zod.z.object({
      profile: smartRecallProfileSchema,
    ...
    smartRecallResultSchema = queryResultSchema.extend({
    ...
    smartRecallResponseSchema = import_zod.z.object({
    ...
    dreamMemoryLayerSchema = import_zod.z.enum(["identity", "semantic", "procedural"]);
    dreamOptionsSchema = import_zod.z.object({
    ...
    dreamResponseSchema = import_zod.z.object({
      query: import_zod.z.string(),
    ...
    contextPackOptionsSchema = smartRecallOptionsSchema.extend({
    ...
    contextPackSectionSchema = import_zod.z.object({
    ...
    contextPackResponseSchema = import_zod.z.object({
      query: import_zod.z.string(),
      profile: smartRecallProfileSchema,
      content: import_zod.z.string(),
    ...
    memoryHealthOptionsSchema = import_zod.z.object({
    ...
    memoryHealthCheckSchema = import_zod.z.object({
    ...
    memoryHealthRecommendationSchema = import_zod.z.object({
      priority: import_zod.z.enum(["low", "medium", "high"]),
      action: import_zod.z.string(),
      reason: import_zod.z.string(),
      command: import_zod.z.string().optional()
    });
    memoryHealthResponseSchema = import_zod.z.object({
    ...
    knowledgeResourceScopeSchema = import_zod.z.string().trim().min(1).max(120).regex(/^[a-zA-Z0-9][a-zA-Z0-9:._/-]*$/, "Knowledge resource scopes must be token-like strings");
    knowledgeResourceGrantSchema = import_zod.z.object({
    ...
    knowledgeNodeSchema = import_zod.z.object({
    ...
    knowledgeEdgeSchema = import_zod.z.object({
    ...
    knowledgeGraphArtifactSchema = import_zod.z.object({
    ...
    knowledgeIngestOptionsSchema = import_zod.z.object({
      source: import_zod.z.string().min(1).optional(),
      project: import_zod.z.string().min(1).optional(),
    ...
    knowledgeIngestResultSchema = import_zod.z.object({
    ...
    knowledgeArtifactRegistrationSchema = import_zod.z.object({
      source: import_zod.z.string().min(1).default("external-knowledge"),
      project: import_zod.z.string().min(1).optional(),
    ...
    modelConfigSchema = import_zod.z.discriminatedUnion("type", [
      import_zod.z.object({
    ...
    embeddingConfigSchema = import_zod.z.object({
    ...
    postgresStorageConfigSchema = import_zod.z.object({
    ...
    storageScopeConfigSchema = import_zod.z.object({
    ...
    rememConfigSchema = import_zod.z.object({
    ...
    eventTypeSchema = import_zod.z.enum([
      "memory.stored",
      "memory.queried",
      "memory.accessed",
      "memory.forgotten",
    ...
    memoryEventSchema = import_zod.z.object({
      id: import_zod.z.string().uuid(),
      type: eventTypeSchema,
      timestamp: import_zod.z.number(),
      payload: import_zod.z.record(import_zod.z.unknown())
    });
    identityCategorySchema = import_zod.z.enum(["values", "boundaries", "preferences", "goals"]);
    constitutionStatementSchema = import_zod.z.object({
      id: import_zod.z.string().uuid(),
    ...
    constitutionSchema = import_zod.z.object({
    ...
    driftResultSchema = import_zod.z.object({
      score: import_zod.z.number().min(0).max(1),
    ...
    identityConfigSchema = import_zod.z.object({
    ...
    memoryLayerSchema = import_zod.z.enum(["episodic", "semantic", "identity", "procedural"]);
    ...
    layeredMemoryEntrySchema = memoryEntrySchema.extend({
    ...
    proceduralTriggerSchema = import_zod.z.object({
    ...
    proceduralMatchSchema = import_zod.z.object({
      entry: layeredMemoryEntrySchema,
      score: import_zod.z.number().min(0).max(2),
      reasons: import_zod.z.array(import_zod.z.string())
    });
    driftEventSchema = import_zod.z.object({
    ...
    identityPackageSchema = import_zod.z.object({
      version: import_zod.z.string().default("1.0"),
    ...
    duplicationConfigSchema = import_zod.z.object({
    ...
    infectionConfigSchema = import_zod.z.object({
      /** DARKSOL server URL */
      serverUrl: import_zod.z.string().url(),
      /** API key for the server */
      apiKey: import_zod.z.string().min(1),
...
// src/embeddings.ts
var import_crypto, OLLAMA_EMBED_URL, EmbeddingService;
...
// src/store.ts
var import_sql, import_crypto2, import_node_fs, MemoryStore;
...
var import_crypto3, PostgresMemoryStore;
...
// src/model.ts
var ModelAbstraction, BankrClient, OpenAIClient, AnthropicClient, OllamaClient;
var init_model = __esm({
  "src/model.ts"() {
    "use strict";
    ModelAbstraction = class {
      client;
      config;
      constructor(config) {
        this.config = config;
...
// src/query.ts
var DEFAULT_SYSTEM_PROMPT, QueryEngine;
var init_query = __esm({
  "src/query.ts"() {
    "use strict";
    DEFAULT_SYSTEM_PROMPT = `You are a memory query assistant. The user has a memory store with entries containing thoughts, facts, preferences, and context.
...
var import_crypto4, ConstitutionManager, DriftDetector, ConstitutionInjector;
...
// src/layers.ts
var import_crypto5, DEFAULT_LAYER_CONFIG, LayerManager;
...
// src/repl.ts
var import_node_vm, DEFAULT_SYSTEM_PROMPT2, MemoryREPL;
var init_repl = __esm({
  "src/repl.ts"() {
    "use strict";
    import_node_vm = require("vm");
    DEFAULT_SYSTEM_PROMPT2 = `You are a memory navigation assistant. The user has a large memory store containing thoughts, facts, preferences, and context.
...
// src/consolidate.ts
var DEFAULT_OPTIONS, MemoryConsolidator;
...
var PROFILE_ALIASES;
var init_recall_profiles = __esm({
  "src/recall-profiles.ts"() {
    "use strict";
    PROFILE_ALIASES = {
...
// src/adapters.ts
function withDefaultTopic(input2, defaultTopic) {
...
function topLevelOwner(entry) {
  ...
  const path3 = getStringMetadata(entry, "path") ?? getStringMetadata(entry, "name") ?? "";
  if (label === "package") return { owner: path3 || "external", type: "package" };
...
function formatCodebaseContext(results, maxChars = 6e3) {
  ...
  for (const result of results) {
    ...
    const path3 = getStringMetadata(result, "path");
    const project = getStringMetadata(result, "project");
    const header = `${label}: ${name}${path3 ? ` (${path3})` : ""}${project ? ` [${project}]` : ""}`;
    lines.push(`- ${header}`);
...
function createVercelAIAdapter(memory, options = {}) {
  return {
    ...
    async remember(input2) {
      const normalized = storeMemoryInputSchema.parse(typeof input2 === "string" ? { content: input2 } : input2);
...
function createHermesAdapter(memory, options = {}) {
  return {
    ...
    async rememberShared(input2) {
      await memory.storeShared({
        content: input2.content,
        namespace: input2.namespace,
        visibility: input2.visibility ?? "shared",
        topics: input2.topics ?? [],
        metadata: {
          ...input2.metadata ?? {},
...
function createCodebaseMemoryAdapter(memory, options = {}) {
  ...
  const summarizeOwners = (nodes, limit) => {
    ...
    for (const node of nodes) {
      ...
      const path3 = getStringMetadata(node, "path");
      ...
      if (path3 && !summary.paths.includes(path3)) summary.paths.push(path3);
  ...
  const summarizeEntrypoints = (nodes, graph, limit) => {
    ...
    for (const node of nodes) {
      ...
      const looksLikeEntrypoint = ["project", "route", "command", "entrypoint", "api"].includes(label) || label === "file" && /\b(cli|server|index|main|app|route|command)s?\b/i.test(`${path3} ${name}`);
  ...
  return {
    ...
    async subgraph(query, queryOptions = {}) {
      ...
      const paths = (response.paths ?? []).filter((path3) => connectionTypeMatches(path3.type, selectedConnectionTypes)).filter((path3) => allowedIds.has(path3.fromId) && allowedIds.has(path3.toId) && allowedIds.has(path3.throughId)).filter((path3) => (queryOptions.minConnectionWeight ?? 0) <= path3.score);
      return {
        query,
    ...
    async explain(query, queryOptions = {}) {
      ...
      const relationTypes = Array.from(new Set(graph.paths.map((path3) => path3.type))).sort();
...
var import_node_crypto, HttpAdapter;
var init_http = __esm({
  "src/http.ts"() {
    "use strict";
    ...
    HttpAdapter = class {
      ...
      async handleRequest(method, url, req) {
        ...
        const runtime = await this.resolveRuntime(method, path3, req);
        const engine = runtime.engine;
        const store = runtime.store;
        const memory = runtime.memory;
        const model = runtime.model;
        if (method === "POST" && path3 === "/memory") {
          ...
          const input2 = storeMemoryInputSchema.parse(JSON.parse(body));
          if (memory?.store) await memory.store(input2);
          else await engine.store(input2);
          return { status: 201, body: { ok: true, message: "Memory stored" } };
        }
        if (method === "POST" && path3 === "/memory/remember") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          if (!body) return { status: 400, body: { error: "Empty request body" } };
          const input2 = rememberInputSchema.parse(JSON.parse(body));
          const result = await memory.remember(input2);
          return { status: result.action === "stored" ? 201 : 200, body: result };
        }
        if (method === "POST" && path3 === "/memory/shared") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          if (!body) return { status: 400, body: { error: "Empty request body" } };
          const parsed = JSON.parse(body);
          const input2 = storeMemoryInputSchema.parse(parsed);
          const namespace = namespaceInputSchema.parse(parsed.namespace);
          const visibility = parsed.visibility === "private" ? "private" : "shared";
          await memory.storeShared({ ...input2, namespace, visibility });
          return { status: 201, body: { ok: true, message: "Shared memory stored", namespace, visibility } };
        }
        if (method === "GET" && path3 === "/memory") {
        ...
        if (method === "POST" && path3 === "/memory/namespace/query") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
        ...
        if (method === "GET" && path3 === "/memory/recent") {
        ...
        if (method === "POST" && path3 === "/memory/namespace/recent") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/memory/query-with-neighbors") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "GET" && path3 === "/memory/recall-profiles") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          return { status: 200, body: { profiles: memory.getRecallProfiles() } };
        }
        if (method === "GET" && path3.startsWith("/memory/recall-profiles/")) {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          const profile = decodeURIComponent(path3.split("/")[3] ?? "").trim();
        ...
        if (method === "POST" && path3 === "/memory/smart-recall") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/memory/context-pack") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if ((method === "GET" || method === "POST") && path3 === "/memory/health") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          let options;
          if (method === "POST") {
            if (!req) return { status: 400, body: { error: "Request body unavailable" } };
        ...
        if (method === "POST" && path3 === "/storage/maintenance") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/artifact") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
        ...
        if (method === "POST" && path3 === "/knowledge/ingest") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/overview") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/access") {
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/subgraph") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/explain") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/graph-memory") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/entrypoints") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/owners") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/hotspots") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/knowledge/deadzones") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/memory/procedural/match") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3 === "/identity/audit") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "GET" && path3.startsWith("/memory/topics/")) {
          const topic = decodeURIComponent(path3.split("/")[3]);
          const limit = parseInt(url.searchParams.get("limit") ?? "20", 10);
          const results = memory?.getByTopic ? await memory.getByTopic(topic, limit) : await engine.getByTopic(topic, limit);
          return { status: 200, body: { results } };
        }
        if (method === "GET" && path3.startsWith("/memory/")) {
          const id = path3.split("/")[2];
        ...
        if (method === "DELETE" && path3.startsWith("/memory/")) {
          const id = path3.split("/")[2];
        ...
        if (method === "POST" && path3 === "/snapshots") {
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path3.startsWith("/snapshots/") && path3.endsWith("/restore")) {
          const id = path3.split("/")[2];
        ...
        if (method === "DELETE" && path3.startsWith("/snapshots/")) {
          const id = path3.split("/")[2];
      ...
      async resolveRuntime(method, path3, req) {
        ...
        const scope = this.buildRequestScope(method, path3, req);
      ...
      buildRequestScope(method, path3, req) {
        const headers = this.buildHeaderMap(req);
        const scope = {
          method,
          path: path3,
...
// src/server.ts
async function readIdentityConstitutionFiles(files) {
  ...
  const texts = await Promise.all(
    uniqueFiles.map(async (file) => {
      ...
      return {
        text: await import_promises.default.readFile(resolved, "utf8"),
        source: import_node_path.default.basename(resolved)
      };
    })
  );
  return texts;
}
async function startReMEMHttpServer(config) {
  const { ReMEM: ReMEM2 } = await Promise.resolve().then(() => (init_index(), index_exports));
  const memory = new ReMEM2(config.memory);
  await memory.init();
...
var import_crypto6, HIGH_IMPORTANCE_PATTERNS, LOW_IMPORTANCE_PATTERNS, TYPE_IMPORTANCE, EpisodicCapturePipeline;
var init_episodic_capture = __esm({
  "src/episodic-capture.ts"() {
    "use strict";
...
__export(index_exports, {
  ...
  readIdentityConstitutionFiles: () => readIdentityConstitutionFiles,
  ...
  startReMEMHttpServer: () => startReMEMHttpServer,
...
var SMART_RECALL_PROFILE_CATALOG, ReMEM;
...
async function writeInitArtifacts(outDir, artifacts) {
  ...
  const files = [
    { path: import_node_path2.default.join(outDir, "remem.config.json"), content: artifacts.configJson },
    { path: import_node_path2.default.join(outDir, "remem-snippet.ts"), content: artifacts.snippetTs },
    { path: import_node_path2.default.join(outDir, ".env.example"), content: artifacts.envExample }
  ];
  await Promise.all(files.map((file) => import_promises3.default.writeFile(file.path, file.content, "utf8")));
...
async function readRememberBatchFile(filePath) {
  const parsed = await readJsonFile(import_node_path2.default.resolve(filePath));
...
async function readKnowledgeGraphFile(filePath) {
  ...
  const data = await import_promises3.default.readFile(resolved);
...
async function packageVersion() {
  const binaryDir = process.argv[1] ? import_node_path2.default.dirname(process.argv[1]) : process.cwd();
  const candidates = [
    import_node_path2.default.resolve(process.cwd(), "package.json"),
    import_node_path2.default.resolve(binaryDir, "..", "package.json")
  ];
...
async function runCli(argv = process.argv, runtime = {}) {
  ...
  if (command === "init") {
    await withMemory(options, async (memory, context) => {
      ...
      const outDir = import_node_path2.default.resolve(asString(options["out-dir"], import_node_path2.default.join(process.cwd(), ".remem")));
      const written = await writeInitArtifacts(outDir, artifacts);
      const configValidation = await validateConfigFile(import_node_path2.default.join(outDir, "remem.config.json"));
      const smokeChecks = await runSmokeChecks(memory, context.config);
      const doctorChecks = options.check || options.doctor ? (await runDoctor(memory, context, { ...options, config: import_node_path2.default.join(outDir, "remem.config.json") })).checks : void 0;
      const payload = {
...
${checkOutput}`);
  ...
  if (command === "serve") {
    ...
    const identityTexts = constitutionFiles.length ? await readIdentityConstitutionFiles(constitutionFiles) : [];
    const serverConfig = {
      ...
      http: {
        ...
        authToken: asString(options["auth-token"]) || void 0,
        corsOrigin: asString(options["cors-origin"]) || void 0,
...
`);
  ...
  if (command === "knowledge-ingest") {
    await withMemory(options, async (memory, context) => {
      ...
      try {
        const stat = await import_promises3.default.stat(resolved);
        if (!stat.isFile()) {
      ...
      const payload = {
        ...
        db: context.dbLabel,
        scope: context.scopeLabel,
        artifact: import_node_path2.default.resolve(artifactPath),
        ...result
      };
      if (jsonMode) emitJson(runtime, payload);
