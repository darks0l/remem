  ...
  "src/http.ts"() {
    ...
    HttpAdapter = class {
      ...
      async handleRequest(method, url, req) {
        ...
        const runtime = await this.resolveRuntime(method, path2, req);
        const engine = runtime.engine;
        const store = runtime.store;
        const memory = runtime.memory;
        const model = runtime.model;
        if (method === "POST" && path2 === "/memory") {
        ...
        if (method === "POST" && path2 === "/memory/remember") {
        ...
        if (method === "POST" && path2 === "/memory/shared") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          if (!body) return { status: 400, body: { error: "Empty request body" } };
        ...
        if (method === "POST" && path2 === "/memory/namespace/query") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
        ...
        if (method === "GET" && path2 === "/memory/recent") {
        ...
        if (method === "POST" && path2 === "/memory/namespace/recent") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/memory/query-with-neighbors") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "GET" && path2 === "/memory/recall-profiles") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          return { status: 200, body: { profiles: memory.getRecallProfiles() } };
        }
        if (method === "GET" && path2.startsWith("/memory/recall-profiles/")) {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          const profile = decodeURIComponent(path2.split("/")[3] ?? "").trim();
        ...
        if (method === "POST" && path2 === "/memory/smart-recall") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/memory/context-pack") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if ((method === "GET" || method === "POST") && path2 === "/memory/health") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          let options;
          if (method === "POST") {
            if (!req) return { status: 400, body: { error: "Request body unavailable" } };
        ...
        if (method === "POST" && path2 === "/storage/maintenance") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/artifact") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
        ...
        if (method === "POST" && path2 === "/knowledge/ingest") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/overview") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/access") {
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/subgraph") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/explain") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/graph-memory") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/entrypoints") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/owners") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/hotspots") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/knowledge/deadzones") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/memory/procedural/match") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2 === "/identity/audit") {
          if (!memory) return { status: 501, body: { error: "Advanced memory runtime not configured" } };
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "GET" && path2.startsWith("/memory/topics/")) {
          const topic = decodeURIComponent(path2.split("/")[3]);
          const limit = parseInt(url.searchParams.get("limit") ?? "20", 10);
          const results = memory?.getByTopic ? await memory.getByTopic(topic, limit) : await engine.getByTopic(topic, limit);
          return { status: 200, body: { results } };
        }
        if (method === "GET" && path2.startsWith("/memory/")) {
        ...
        if (method === "DELETE" && path2.startsWith("/memory/")) {
          const id = path2.split("/")[2];
        ...
        if (method === "POST" && path2 === "/snapshots") {
          if (!req) return { status: 400, body: { error: "Request body unavailable" } };
          const body = await this.readBody(req);
          const parsed = body ? JSON.parse(body) : {};
        ...
        if (method === "POST" && path2.startsWith("/snapshots/") && path2.endsWith("/restore")) {
          const id = path2.split("/")[2];
        ...
        if (method === "DELETE" && path2.startsWith("/snapshots/")) {
          const id = path2.split("/")[2];
      ...
      async resolveRuntime(method, path2, req) {
        ...
        const scope = this.buildRequestScope(method, path2, req);
      ...
      buildRequestScope(method, path2, req) {
        const headers = this.buildHeaderMap(req);
        const scope = {
          method,
          path: path2,
